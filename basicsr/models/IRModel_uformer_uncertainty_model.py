...
class IRModel_Uformer_uncertainty(BaseModel):   #纯净原版
    """Base IR model, it will recored 7 type of losses."""

    def __init__(self, opt):
        super(IRModel_Uformer_uncertainty, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_pix:
            ...
            if current_iter %1 ==0:
                ...
                l_total,loss_list,old_loss_dict,uncertainty_loss_dict,uncertainty_bn_dict=self.cri_pixtype(self.output, self.gt,self.typeword,self.uncertanty)
                loss_dict['l_pix'] = l_total
                if loss_list[0] != 0:
                    self.loss_blur = loss_list[0].detach()

                if loss_list[1] != 0:
                    self.loss_noise = loss_list[1].detach()

                if loss_list[2] != 0:
                    self.loss_jpeg = loss_list[2].detach()

                if loss_list[3] != 0:
                    self.loss_rain = loss_list[3].detach()

                if loss_list[4] != 0:
                    self.loss_haze = loss_list[4].detach()

                if loss_list[5] != 0:
                    self.loss_dark = loss_list[5].detach()
                
                if loss_list[6] != 0:
                    self.loss_sr = loss_list[6].detach()
                ...
                old_loss_name = ['old_loss_blur','old_loss_noise','old_loss_jpeg','old_loss_rain','old_loss_haze','old_loss_dark','old_loss_sr']
                uncertainty_loss_name = ['uncertainty_loss_blur','uncertainty_loss_noise','uncertainty_loss_jpeg','uncertainty_loss_rain','uncertainty_loss_haze','uncertainty_loss_dark','uncertainty_loss_sr']
                uncertainty_bn_name = ['uncertainty_bn_blur','uncertainty_bn_noise','uncertainty_bn_jpeg','uncertainty_bn_rain','uncertainty_bn_haze','uncertainty_bn_dark','uncertainty_bn_sr']
                i = 0
    ...
    def test(self):
        ...
        self.net_g.eval()
        with torch.no_grad():
            self.output,self.uncertanty = self.net_g(img)
        self.net_g.train()
