...
class Uformer_uncertainty(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 ...
                 cross_modulator=False, if_uncertainty = True,**kwargs):
        super().__init__()
        ...
        self.var_conv = nn.Sequential(
            ...
            nn.Conv2d(64, 64, 3, 1, 1), nn.ELU(),
            nn.Conv2d(64, 3, 3, 1, 1),nn.Softplus()
        )
    ...
    def forward(self, x, mask=None):
        _, _, H, W = x.shape
        '''
        if H != W or H%128 !=0 or W%128 != 0:
            #print('padding')
            x_pad, mask = expand2square(x, factor=128)
            #print('after expand: ', x_pad.shape)
        else:
            x_pad = x
        '''
        x_pad = x
        # Input Projection
        y = self.input_proj(x_pad)
        y = self.pos_drop(y)
        #Encoder
        ...
        if self.if_uncertainty:
            ...
            H = int(math.sqrt(L))
            W = int(math.sqrt(L))
            deconv3_for_un = deconv3.transpose(1, 2).view(B, C, H, W)

            #print(deconv3_for_un.shape)
            un = self.var_conv(deconv3_for_un)
            #print('success un')
...
class Uformer_EP(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.F_ext_net5 = F_ext(in_nc=1, nf=64)

        dim=embed_dim

        # self.prompt_scale1 = nn.Linear(prompt_nf, dim, bias=True)
        # self.prompt_scale2 = nn.Linear(prompt_nf, int(dim * 2 ** 1), bias=True)
        # self.prompt_scale3 = nn.Linear(prompt_nf, int(dim * 2 ** 2), bias=True)
        # self.prompt_scale4 = nn.Linear(prompt_nf, int(dim * 2 ** 3), bias=True)

        self.prompt_scale0 = nn.Linear(prompt_nf, int(dim * 2 ** 4), bias=True)

        self.prompt_scale1 = nn.Linear(prompt_nf, int(dim * 2 ** 4), bias=True)
        self.prompt_scale2 = nn.Linear(prompt_nf, int(dim * 2 ** 3), bias=True)
        self.prompt_scale3 = nn.Linear(prompt_nf, int(dim * 2 ** 2), bias=True)
        self.prompt_scale4 = nn.Linear(prompt_nf, int(dim * 2 ** 1), bias=True)

        self.prompt_scale5 = nn.Linear(prompt_nf, 3, bias=True)
 

        # self.prompt_shift1 = nn.Linear(prompt_nf, dim, bias=True)
        # self.prompt_shift2 = nn.Linear(prompt_nf, int(dim * 2 ** 1), bias=True)
        # self.prompt_shift3 = nn.Linear(prompt_nf, int(dim * 2 ** 2), bias=True)
        # self.prompt_shift4 = nn.Linear(prompt_nf, int(dim * 2 ** 3), bias=True)

        self.prompt_shift0 = nn.Linear(prompt_nf, int(dim * 2 ** 4), bias=True)

        self.prompt_shift1 = nn.Linear(prompt_nf, int(dim * 2 ** 4), bias=True)
        self.prompt_shift2 = nn.Linear(prompt_nf, int(dim * 2 ** 3), bias=True)
        self.prompt_shift3 = nn.Linear(prompt_nf, int(dim * 2 ** 2), bias=True)
        self.prompt_shift4 = nn.Linear(prompt_nf, int(dim * 2 ** 1), bias=True)
        self.prompt_shift5 = nn.Linear(prompt_nf, 3, bias=True)
    ...
    def forward(self, x, mask=None):
        ...
        E_prompt = torch.unsqueeze(x[1], 1)

        prompt1 = self.F_ext_net1(E_prompt)
        prompt2 = self.F_ext_net2(E_prompt)
        prompt3 = self.F_ext_net3(E_prompt)
        prompt4 = self.F_ext_net4(E_prompt)
        prompt5 = self.F_ext_net5(E_prompt)
        ...
        scale0 = self.prompt_scale0(prompt1)
        shift0 = self.prompt_shift0(prompt1)
        ...
        scale4 = self.prompt_scale4(prompt4)
        shift4 = self.prompt_shift4(prompt4)

        scale5 = self.prompt_scale5(prompt5)
        shift5 = self.prompt_shift5(prompt5)
        ...
        #Encoder
        conv0 = self.encoderlayer_0(y,mask=mask)
        #conv0 = conv0 * scale1.view(-1, 1, self.embed_dim) + shift1.view(-1, 1, self.embed_dim) + conv0

        pool0 = self.dowsample_0(conv0)
        conv1 = self.encoderlayer_1(pool0,mask=mask)
        #conv1 = conv1 * scale2.view(-1, 1,self.embed_dim*2) + shift2.view(-1, 1,self.embed_dim*2) + conv1
        pool1 = self.dowsample_1(conv1)
        conv2 = self.encoderlayer_2(pool1,mask=mask)
        #conv2 = conv2 * scale3.view(-1, 1, self.embed_dim*4) + shift3.view(-1, 1, self.embed_dim*4) + conv2
        pool2 = self.dowsample_2(conv2)
        conv3 = self.encoderlayer_3(pool2,mask=mask)
        #conv3 = conv3 * scale4.view(-1, 1, self.embed_dim*8) + shift4.view(-1, 1, self.embed_dim*8) + conv3
        pool3 = self.dowsample_3(conv3)

        # Bottleneck
        conv4 = self.conv(pool3, mask=mask)
        conv4 = conv4 * scale0.view(-1, 1, self.embed_dim*16) + shift0.view(-1, 1, self.embed_dim*16) + conv4

        #Decoder
        up0 = self.upsample_0(conv4)
        deconv0 = torch.cat([up0,conv3],-1)
        deconv0 = self.decoderlayer_0(deconv0,mask=mask)
        deconv0 = deconv0 * scale1.view(-1, 1, self.embed_dim*16) + shift1.view(-1, 1, self.embed_dim*16) + deconv0
        
        up1 = self.upsample_1(deconv0)
        deconv1 = torch.cat([up1,conv2],-1)
        deconv1 = self.decoderlayer_1(deconv1,mask=mask)
        deconv1 = deconv1 * scale2.view(-1, 1, self.embed_dim*8) + shift2.view(-1, 1, self.embed_dim*8) + deconv1

        up2 = self.upsample_2(deconv1)
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.decoderlayer_2(deconv2,mask=mask)
        deconv2 = deconv2 * scale3.view(-1, 1, self.embed_dim*4) + shift3.view(-1, 1, self.embed_dim*4) + deconv2

        up3 = self.upsample_3(deconv2)
        deconv3 = torch.cat([up3,conv0],-1)
        deconv3 = self.decoderlayer_3(deconv3,mask=mask)
        deconv3 = deconv3 * scale4.view(-1, 1,self.embed_dim*2) + shift4.view(-1, 1,self.embed_dim*2) + deconv3

        # Output Projection
        y = self.output_proj(deconv3)
        y = y * scale5.view(-1, 3, 1, 1) + shift5.view(-1, 3, 1, 1) + y
        return x + y if self.dd_in ==3 else y
        '''
        if H != W or H%128 !=0 or W%128 != 0:
            y = torch.masked_select(y,mask.bool()).reshape(1,3,H,W)
...
class Uformer_AP(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.F_ext_net5 = F_ext(in_nc=3, nf=64)
    ...
    def forward(self, x, mask=None):
        ...
        prompt1 = self.F_ext_net1(A_prompt)
        prompt2 = self.F_ext_net2(A_prompt)
        prompt3 = self.F_ext_net3(A_prompt)
        prompt4 = self.F_ext_net4(A_prompt)
        prompt5 = self.F_ext_net5(A_prompt)
