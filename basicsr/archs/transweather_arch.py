...
from basicsr.archs.transweather.base_networks import *
...
class EncoderTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x2, H2, W2 = self.mini_patch_embed1(
            x1.permute(0, 2, 1).reshape(B, embed_dims[0], H1, W1)
        )
...
class Transweather_FT(nn.Module):

    def __init__(
        self,
    ):
        super(Transweather_FT, self).__init__()
...
class Transweather(nn.Module):
    ...
    def __init__(self, path=None, if_uncertainty = True,**kwargs):
        ...
        self.var_conv = nn.Sequential(
            ...
            nn.Conv2d(8, 8, 3, 1, 1), nn.ELU(),
            nn.Conv2d(8, 3, 3, 1, 1),nn.Softplus()
        )  
    def forward(self, x):
